...
exports.cvDataToText = cvDataToText;
exports.checkATSRules = checkATSRules;
...
function cvDataToText(cvData) {
    ...
    if (Array.isArray(cvData.skills)) {
        ...
        for (const skill of cvData.skills) {
            const s = skill;
            const skillList = Array.isArray(s.skills) ? s.skills.join(', ') : '';
    ...
    if (Array.isArray(cvData.languages_spoken)) {
        ...
        for (const lang of cvData.languages_spoken) {
            const l = typeof lang === 'string' ? { language: lang } : lang;
            parts.push(`- ${l.language || l.name || ''} (${l.level || ''})`);
        }
    }
    return parts.join('\n');
}
function checkATSRules(content, jobDescription) {
    const sections = [];
    ...
    if (verbCount < 5)
        recommendations.push(`Use more action verbs (found ${verbCount}, aim for 5+)`);
    ...
    if (!hasQuantified)
        recommendations.push('Add quantified achievements (%, $$, numbers) to strengthen impact');
    ...
    if (jobDescription && jobDescription.trim()) {
        ...
        const jdUniqueWords = new Set();
        for (const w of jdWords) {
            if (!cvWords.has(w) && w.length > 3)
                jdUniqueWords.add(w);
        }
    ...
    if (typoMatches.length > 0)
        recommendations.push(`Fix possible typos: ${typoMatches.join(', ')}`);
